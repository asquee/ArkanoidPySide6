...
class ArkanoidMainWindow(QMainWindow):
    ...
    def show_menu_screen(self):
        self.stacked_widget.setCurrentIndex(0)  # Индекс 0 соответствует экрану меню
...
class Bonus(QGraphicsRectItem):
    ...
    def move(self):
        self.setY(self.y() + 2)  # бонусы падают вниз
...
class ArkanoidGame(QGraphicsView, QWidget):
    def __init__(self):
        ...
        self.life = 3
        self.life_indicators = []  # Список для размещения кружочков с индикаторами жизни
        ...
        self.bricks = []
        self.bonuses = []
        self.generate_bricks() # # Вызов функции generate_bricks для случайного размещения

        self.game_started = False  # Флажок для отслеживания начала игры
        self.timer = QTimer()
    ...
    def generate_bricks(self):
        # Определение свойств кирпича (ширина, высота, расстояние между ними)
        brick_width = 75
        brick_height = 20
        brick_spacing_h = 4
        brick_spacing_v = 2
        ...
        for row in range(num_rows):
            for col in range(num_cols):
                # Генерация блоков случайным образом
                if random.random() < 0.7:
                    ...
                    is_gray = random.random() < 0.1 # шанс генерации серго блока
                    ...
                    if is_gray:
                        brick.setBrush(Qt.gray) #другой цвет для неразрушимых кирпичей
    ...
    def update(self):
        if not self.game_started:
            return  # игра не запущена, логика игры не обновляется
        ...
        for brick in self.bricks:
            ...
            if self.ball.collidesWithItem(brick):
                # Проверьте, не стал ли кирпич серым (неразрушимым).
                if brick.brush().color() == Qt.gray:
                    ...
                    if random.random() < 0.1:  # 10% выпадение бонусов
    ...
    def apply_bonus(self, bonus_type):
        if bonus_type == 1 and self.platform.rect().width() <= 140:
            ...
            self.ball.setBrush(Qt.red)  #  мяч может разбивать серые кирпичи
